...
import {sum_array,group_ready,unhide_value,hide_value,get_date,sliceStringFromValue} from "../src/utils.mjs"
...
export function get_all_groups(){
	...
	for (const group of [...rheos_groups.values()]){
		console.log("SETTING FROM RHEOS GROUP",group)
		all_groups.set(group.sum_group,group)
	}
	console.log("GETTING FIXED GROUPS",fixed_groups)
	for (const group of [...fixed_groups.values()]){
		console.log("SETTING FROM FIXED GROUP",group)
		all_groups.set(group.sum_group,group)
	}
...
export async function group_enqueue(group,sum_group) {
	if (Array.isArray(group) && (group = group.filter(o => o))){
		if (group  && ([...rheos_groups.values()].findIndex((g) => g.sum_group == sum_group)==-1)){
